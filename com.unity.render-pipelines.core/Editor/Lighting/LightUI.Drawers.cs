...
namespace UnityEditor.Rendering
{
    ...
    {
        /// <summary>
        /// Draws the color UI for a given light
        /// </summary>
        /// <param name="serialized"><see cref="ISerializedLight"/>The light to draw the color UI</param>
        /// <param name="owner"><see cref="Editor"/>The editor that is handling the light</param>
            ...
            using (var changes = new EditorGUI.ChangeCheckScope())
            {
                if (GraphicsSettings.lightsUseLinearIntensity && GraphicsSettings.lightsUseColorTemperature)
                {
                    ...
                    if (serialized.settings.useColorTemperature.boolValue)
                    {
                        ...
                        TemperatureSliderSliderUIDrawer.Draw(serialized.settings, serialized.serializedObject, serialized.settings.colorTemperature, temperatureSliderRect);
                        ...
                        EditorGUI.PropertyField(valueRect, serialized.settings.colorTemperature, Styles.empty);
                        EditorGUI.Popup(unitRect, 0, Styles.lightAppearanceUnits);
