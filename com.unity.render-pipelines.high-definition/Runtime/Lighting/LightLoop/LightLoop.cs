...
namespace UnityEngine.Rendering.HighDefinition
{
    ...
    public partial class HDRenderPipeline
    {
        ...
        internal HDGpuLightList gpuLightList => m_GpuLightList;
        ...
        void InitializeLightLoop(IBLFilterBSDF[] iBLFilterBSDFArray)
        {
            ...
            m_GpuLightList.Initialize(m_Asset, m_ShadowManager, m_TextureCaches);
        ...
        void GetEnvLightVolumeDataAndBound(HDProbe probe, LightVolumeType lightVolumeType, Matrix4x4 worldToView, int viewIndex)
        {
            ...
            m_GpuLightList.AddLightBounds(viewIndex, bound, lightVolumeData);
        }
        ...
        void PrepareGPULightdata(CommandBuffer cmd, HDCamera hdCamera, CullingResults cullResults, in HDVisibleLightEntities visibleLightEntities)
        {
            using (new ProfilingScope(cmd, ProfilingSampler.Get(HDProfileId.PrepareGPULightdata)))
            {
                ...
                m_CurrentSunLightAdditionalLightData = m_GpuLightList.currentSunLightAdditionalLightData;
                m_CurrentSunShadowMapFlags = m_GpuLightList.currentSunShadowMapFlags;
                m_CurrentSunLightDirectionalLightData = m_GpuLightList.currentSunLightDirectionalLightData;

                m_ContactShadowIndex = m_GpuLightList.contactShadowIndex;
                m_ScreenSpaceShadowIndex = m_GpuLightList.screenSpaceShadowIndex;
                m_ScreenSpaceShadowChannelSlot = m_GpuLightList.screenSpaceShadowChannelSlot;
                m_DebugSelectedLightShadowIndex = m_GpuLightList.debugSelectedLightShadowIndex;
                m_DebugSelectedLightShadowCount = m_GpuLightList.debugSelectedLightShadowCount;
                m_CurrentScreenSpaceShadowData = m_GpuLightList.currentScreenSpaceShadowData;
            }
        }
            ...
            using (new ProfilingScope(cmd, ProfilingSampler.Get(HDProfileId.PrepareLightsForGPU)))
            {
                ...
                if (decalDatasCount > 0)
                {
                    for (int i = 0; i < decalDatasCount; i++)
                    {
                        for (int viewIndex = 0; viewIndex < hdCamera.viewCount; ++viewIndex)
                        {
                            m_GpuLightList.AddLightBounds(viewIndex, DecalSystem.m_Bounds[i], DecalSystem.m_LightVolumes[i]);
                        }
                    }
                }
                ...
                for (int viewIndex = 0; viewIndex < hdCamera.viewCount; ++viewIndex)
                {
                    ...
                    for (int i = 0, n = m_LocalVolumetricFogCount; i < n; i++)
                    {
                        ...
                        m_GpuLightList.AddLightBounds(viewIndex, bound, volumeData);
                    }
                }

                m_TotalLightCount = m_GpuLightList.lightsCount + m_lightList.envLights.Count + decalDatasCount + m_LocalVolumetricFogCount;

                Debug.Assert(m_TotalLightCount == m_GpuLightList.lightsPerView[0].boundsCount);
        ...
        unsafe void UpdateShaderVariablesGlobalLightLoop(ref ShaderVariablesGlobal cb, HDCamera hdCamera)
        {
            ...
            // Light info
            cb._PunctualLightCount = (uint)m_GpuLightList.punctualLightCount;
            cb._AreaLightCount = (uint)m_GpuLightList.areaLightCount;
            cb._EnvLightCount = (uint)m_lightList.envLights.Count;
            cb._DirectionalLightCount = (uint)m_GpuLightList.directionalLightCount;
        ...
        {
            m_LightLoopLightData.directionalLightData.SetData(m_GpuLightList.directionalLights, 0, 0, m_GpuLightList.directionalLightCount);
            m_LightLoopLightData.lightData.SetData(m_GpuLightList.lights, 0, 0, m_GpuLightList.lightsCount);
            ...
            m_TileAndClusterData.convexBoundsBuffer.SetData(m_GpuLightList.lightBounds, 0, 0, m_GpuLightList.allLightBoundsCount);
            m_TileAndClusterData.lightVolumeDataBuffer.SetData(m_GpuLightList.lightVolumes, 0, 0, m_GpuLightList.allLightBoundsCount);
